...
// look up our keys?
        ...
        server.on("/testSave", HTTP_POST, [](PsychicRequest *request, JsonVariant &json) {
            // Because it can split out some strings which is valid JSON
            static const int MIN_LENGTH = 20;
            Serial.println();
            int len = serializeJsonPretty(json, Serial);
            Serial.println();
            return request->reply(len>MIN_LENGTH?200:500, "text/plain", len>MIN_LENGTH?"OK":"Failed");
        });
